...
// - Add div between icon and product name
		...
		if($check > 0){
			...
			foreach ($eshopcartarray as $productid => $opt){
				...
				if(is_array($opt)){
					...
					$textdesc='<a href="'.get_permalink($opt['postid']).'">'.$opt['pid'].$variation.'</a>'.$echooptset;
					// -- cs
					$echoimg=$eimg;
          ...
          $echo.= '<td id="prod'.$calt.$iswidget.'" headers="cartItem'.$iswidget.'" class="leftb cartitem"><div id="icon">'.$echoimg.'</div><div id="title">'.$textdesc.'<div></td>'."\n";
