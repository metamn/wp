		...
		if($stkav=='1'){
							...
							case '0'://select
								...
								foreach($optsets['item'] as $opsets){
									if($opsets['price']!='0.00')
										// by cs
										//$addprice=sprintf( __(' + %1$s%2$s','eshop'), $currsymbol, number_format_i18n($opsets['price'],__('0','eshop')));
										$addprice=sprintf( __(' + %1$s %2$s','eshop'), number_format_i18n($opsets['price'],__('0','eshop')), $currsymbol);
									else
										$addprice='';
			...
			if($eshopoptions['options_num']>1 && !empty($eshop_product['products']['2']['option']) && !empty($eshop_product['products']['2']['price'])){
				if(isset($eshop_product['cart_radio']) && $eshop_product['cart_radio']=='1'){
					...
					for($i=1;$i<=$opt;$i++){
						...
						if($option!='' && $currst>0){
							if($price!='0.00')
								$mainoptsecho.='<li><input type="radio" value="'.$i.'" id="eshopopt'.$theid.'_'.$i.$uniq.'" name="option"'.$esel.' /><label for="eshopopt'.$theid.'_'.$i.$uniq.'">'.sprintf( __('%1$s -- %2$s %3$s','eshop'),stripslashes(esc_attr($option)), number_format_i18n($price,__('0','eshop')), $currsymbol)."</label>\n</li>";
							else
			...
			*/
			...
			if($short=='yes'){
				...
				<input type="hidden" value="'.$addqty.'" id="qty'.$theid.$uniq.'" maxlength="3" size="3" name="qty" class="iqty" />';
			}
